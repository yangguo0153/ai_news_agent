...
class WordGenerator:
    ...
    def add_images_to_page(self, image_paths: List[str], title: str = "", max_per_row: int = 2, compact: bool = False):
        """
        将多张图片添加到文档

        :param image_paths: 图片路径列表
        :param title: 标题（可选）
        :param max_per_row: 每行最多图片数
        :param compact: 是否使用紧凑模式（缩小图片尺寸）
        """
        if title:
        ...
        # 紧凑模式下进一步缩小
        if compact:
            img_width = min(img_width, 7)  # 最大 7cm
            single_img_width = 8  # 单张图片 8cm
        else:
            single_img_width = 12  # 单张图片 12cm
        ...
        for i in range(0, len(image_paths), max_per_row):
            ...
            if len(batch) > 1:
                ...
                run = paragraph.add_run()
                try:
                    run.add_picture(batch[0], width=Cm(single_img_width))
                except Exception as e:
    ...
    def add_invoice_full_width(self, image_path: str):
        """
        添加全宽发票（竖向，最大化宽度）
        不再旋转，避免横版溢出
        """
        ...
        try:
            # 添加图片，宽度设为页面可用最大宽度 (约16cm / 6.3英寸)
            # A4 width 8.27 - 1.0 (margins) = 7.27, 保险起见用 6.5
            self.doc.add_picture(str(image_path), width=Inches(6.5))
            
            # 每张发票后自动分页
            self.add_page_break()
            
        except Exception as e:
            print(f"⚠️ 添加全宽图片失败 {image_path}: {e}")
            # 降级
            try:
